...
| [#73](https://github.com/Calyx-Engineering/arc/issues/73) | The autonomy switch — specified and built | **m40** autonomy switch | [m40](../product-architecture/mechanisms/m40-autonomy-switch.md) — **this issue produced it**, with [`skills/autonomy-set`](../../skills/autonomy-set/SKILL.md) and `tests/verify-autonomy.sh`. It returned from *Onboarding — m47*; retitled from `spec:` because the scope became spec-and-build |
...
| 15 | 5.2 | [#32](https://github.com/Calyx-Engineering/arc/issues/32) | Size an issue title to what merging delivers | **Merged** — [PR #97](https://github.com/Calyx-Engineering/arc/pull/97). Ships `tests/verify-tracker-body.sh title` and `issue-write`'s Titles section |
...
| 22 | 6.2 | [#73](https://github.com/Calyx-Engineering/arc/issues/73) | The autonomy switch — specified and built | **Merged** — [PR #116](https://github.com/Calyx-Engineering/arc/pull/116). Left for *Onboarding — m47* and **returned 2026-08-21** after three to five failed surface fixes; it reclaims its own slot, so nothing renumbers. Ships [m40](../product-architecture/mechanisms/m40-autonomy-switch.md), `skills/autonomy-set` and `tests/verify-autonomy.sh` |
| — | — | [PR #118](https://github.com/Calyx-Engineering/arc/pull/118) | One command runs every gate | **Merged** 08-21 20:02. `tests/verify-all.sh` — 8 gates, 112 cases at [#78](https://github.com/Calyx-Engineering/arc/issues/78). Prerequisite for [#117](https://github.com/Calyx-Engineering/arc/issues/117) |
...
| — | — | [#122](https://github.com/Calyx-Engineering/arc/issues/122) | Template links break where the template lands | **Merged** — [PR #128](https://github.com/Calyx-Engineering/arc/pull/128). The review's one blocking finding, fixed before the release. 18 links, five templates. Ships `tests/verify-template-links.sh` — the ninth gate — and `templates/README.md` |
| 25 | 6.5 | [#120](https://github.com/Calyx-Engineering/arc/issues/120) | Release `v0.1.0` | **Merged** — [PR #129](https://github.com/Calyx-Engineering/arc/pull/129). Ships `.claude-plugin/marketplace.json`, which did not exist and which **both** install routes need, and [`docs/release/release-process.md`](../release/release-process.md). **Installing it is the user's** |
| — | — | [PR #130](https://github.com/Calyx-Engineering/arc/pull/130) | The arc close — soak, friction sweep, status | This row. §14's checklist |
...
| `tests/verify-all.sh` — the gate runner ([PR #118](https://github.com/Calyx-Engineering/arc/pull/118)) | [#78](https://github.com/Calyx-Engineering/arc/issues/78), then every PR of wave 6 | **Ran clean, six times.** It replaced four invocations from memory, which is the whole claim. **It found none of the pre-release review's six findings** — correct, and the argument for the review rather than against the runner: every finding is on a surface no gate covered. **`--list` is the half worth keeping**, because it prints what the runner cannot cover |
| `tests/verify-template-links.sh` — the destination resolver ([#122](https://github.com/Calyx-Engineering/arc/issues/122)) | Its own two mutations, then [PR #129](https://github.com/Calyx-Engineering/arc/pull/129) and [PR #130](https://github.com/Calyx-Engineering/arc/pull/130) | **Fired correctly on both mutations before it was trusted.** A link broken at the destination is caught with the resolved path named; a template with no map row fails by name rather than being skipped. **The gate that has never failed is the gate nobody has tested** — this one was made to fail twice on purpose |
| `docs/release/pre-release-review.md` — the four passes ([#119](https://github.com/Calyx-Engineering/arc/issues/119)) | Its own first run, against every shipping artifact | **Six findings, one blocking, and the blocking one was only reachable through pass 2's *resolve from the destination*** — which the issue's checklist did not ask for and which writing the process before running it produced. **Not soaked as a repeatable process**: one run does not show a second reviewer reaches the same findings |
| `.claude-plugin/marketplace.json` · `docs/release/release-process.md` ([#120](https://github.com/Calyx-Engineering/arc/issues/120)) | — | **Unsoaked, and it is the one that matters now.** Both are verified against the plugin and marketplace references and against nothing else. **The first install is the soak**, and it is what turns every behavioural criterion in three arcs from asserted into observed |

| **Waves 1–4 entire** — Camp's three documents, the event log, reaching Camp, the report, plan modes, edit verification, the handoff's ordered actions, status and the close sequence, the relief valve, decomposition, the nudge, verbosity, the intent check | — | **Unsoaked, all of it, and one row rather than thirteen.** Every one is a skill, a hook or a document a skill reads, and **nothing in this repository invokes a skill or fires a hook.** Thirteen rows each saying so would read as thirteen findings; it is one fact. The exceptions already have their own rows: [#42](https://github.com/Calyx-Engineering/arc/issues/42)'s `arc-intent` was fired by hand, and wave 3's hooks were executed against fixtures by `tools/verify-hook.sh`. **`v0.1.0` and an install is what closes this**, for all thirteen at once |
...
- [x] [#119](https://github.com/Calyx-Engineering/arc/issues/119) merged — [PR #127](https://github.com/Calyx-Engineering/arc/pull/127). Six findings, one blocking, **filed rather than fixed in place**
- [x] [#122](https://github.com/Calyx-Engineering/arc/issues/122) merged — [PR #128](https://github.com/Calyx-Engineering/arc/pull/128). The blocking finding, fixed before the release rather than released over
- [x] [#120](https://github.com/Calyx-Engineering/arc/issues/120) merged — [PR #129](https://github.com/Calyx-Engineering/arc/pull/129). `v0.1.0`, both install routes, and the marketplace file neither route had
- [x] Status table reflects reality — §10, every row, including the four direct PRs of wave 6
- [x] Soak line appended for every plugin change made during this arc — **waves 1–4 are one consolidated row**, not thirteen: they are unsoaked for a single reason, and the release is what closes all of them at once
- [x] `docs/arc-work/03-camp/friction-log.md` swept — see §14.1
- [x] [#105](https://github.com/Calyx-Engineering/arc/issues/105) · [#106](https://github.com/Calyx-Engineering/arc/issues/106) confirmed open on *Self-improvement*, joined by [#117](https://github.com/Calyx-Engineering/arc/issues/117) and the review's four non-blocking findings [#123](https://github.com/Calyx-Engineering/arc/issues/123)–[#126](https://github.com/Calyx-Engineering/arc/issues/126)
- [x] K2 swept — the friction log is this arc's K2 and dies with it; §14.1 is what graduated
- [ ] Arc PR into `main` — **no closing lines needed.** [m42](../product-architecture/mechanisms/m42-default-branch-flip.md)'s flip made every PR bind its own issue as it merged, so the milestone is already at **65 closed, 0 open**. The old requirement predates the flip
- [ ] Default branch restored — `tools/arc-default-branch.sh restore`, **after** the arc PR merges
- [ ] `v0.1.0` tagged on `main`, and the GitHub release cut — [`release-process.md`](../release/release-process.md) §4's order

### 14.1 The friction sweep

**Four entries. Two were resolved inside the arc, and two carried unfiled items that are now
filed or deliberately dropped.**

| Entry | | Where it went |
|---|---|---|
| **1 · the merge step never executed, and the first diagnosis was wrong** | Resolved. The variable is **who initiated**, and [#73](https://github.com/Calyx-Engineering/arc/issues/73) built [m40](../product-architecture/mechanisms/m40-autonomy-switch.md) from it. **Wave 6.3 added the fact that closes it**: one standing grant covered a whole issue's merges, where every earlier success needed an ask per merge | m40 · the *Soak* row above |
| **2 · the agent cannot install its own autonomy switch** | Resolved. m40 §7 states it as a design constraint — *an agent that can install its own switch has no switch* — and `skills/autonomy-set` repeats it. The *unfiled — ask* is now moot | m40 §7 |
| **3 · the staleness check false-positived on every cold start** | Resolved by [PR #115](https://github.com/Calyx-Engineering/arc/pull/115). Compare mtimes, never the curated table | `commands/arc-next.md` · m15 |
| **4 · the merge denied again with the allow-list in place** | Resolved by entry 1's answer, and **superseded by wave 6.3's data point.** Its *"unfiled"* is dropped rather than filed: the thing it wanted filed was a durable switch, and that exists | m40 |

**What did not graduate, and why.** *§6.1.1 step 10 has no failure path* — it does now, in m40:
retry once, record it in three places, hand it over. *The handoff has no home for what was ruled
out* — still true, and it is [#105](https://github.com/Calyx-Engineering/arc/issues/105)'s
territory rather than a fifth entry.

> **The log dies with the arc, and that was always the design.** It is the raw material a
> retrospective mines, not a small copy of one. What outlives it is in m40, m15, and the four
> rows above.

...
| **Tracker** | [#147](https://github.com/Calyx-Engineering/arc/issues/147) | 7 | **In progress.** [#199](https://github.com/Calyx-Engineering/arc/issues/199) — `tools/verify-issue-boxes.sh`, the box count `hooks/tracker-verify` runs on `gh pr ready` — is built. [#140](https://github.com/Calyx-Engineering/arc/issues/140)'s *not a gate script* constraint is narrowed to the prose half |
...
| `tools/verify-issue-boxes.sh` and `hooks/tracker-verify`'s `gh pr ready` check ([#199](https://github.com/Calyx-Engineering/arc/issues/199)) | **Its own unit, live** — run against [#140](https://github.com/Calyx-Engineering/arc/issues/140) and [#199](https://github.com/Calyx-Engineering/arc/issues/199) as real issues before either was ticked | **Fired correctly on the live path, and the hook half is unsoaked.** The tool read #140's nine ticked boxes as clean and #199's seven open ones as findings, over the real GraphQL reads the selftest can only fixture. The hook is the same problem [#166](https://github.com/Calyx-Engineering/arc/issues/166)'s row records — the installed plugin is the main tree, and a running session does not pick up a reload — so **this PR's own `gh pr ready` is its first exercise**, and the thing to read is whether `.claude/arc/log.md` gains a `pr-ready` entry carrying a real PR number |

...
| **Fire** | [#145](https://github.com/Calyx-Engineering/arc/issues/145) | 13 | **In progress.** [#163](https://github.com/Calyx-Engineering/arc/issues/163) closed; [#166](https://github.com/Calyx-Engineering/arc/issues/166) — the activation log, every hook leaving a record — is in [#244](https://github.com/Calyx-Engineering/arc/pull/244). Spawned [#238](https://github.com/Calyx-Engineering/arc/issues/238) and [#239](https://github.com/Calyx-Engineering/arc/issues/239) |
...
| **Upkeep** | [#148](https://github.com/Calyx-Engineering/arc/issues/148) | 9 | **In progress.** [#201](https://github.com/Calyx-Engineering/arc/issues/201) — `hooks/mode-guard` reading what a script does rather than what the command says — is in [#247](https://github.com/Calyx-Engineering/arc/pull/247) |
...
| `hooks/lib/activation-log` and all six hooks ([#166](https://github.com/Calyx-Engineering/arc/issues/166)) | — | **Unsoaked, and it cannot be soaked from inside its own run.** The installed plugin is the main tree, which does not carry the library, and `tools/plugin-reload.sh` says a running session does not pick up a reload — so no hook has fired in a real session with this code. Everything asserted is the harness: 37 gates clean, 392 assertions over 115 cases, and a selftest that proves the gate can fail. **Its first exercise was [#201](https://github.com/Calyx-Engineering/arc/issues/201)'s run**, in a worktree with the plugin installed: `.claude/arc/log.md` took ~1,800 lines of entries carrying real commands and real paths, not fixtures — which is exactly the distinction the first version of this library got wrong. **Still unread: whether the entries are useful**, and the log is uncommitted runtime state, so no PR carries it |

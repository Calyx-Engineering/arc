...
| **Unavailable** | `claude plugin eval` | `` `plugin eval` is currently in early access ``, exit 1 — [#181](https://github.com/Calyx-Engineering/arc/issues/181). **Re-checked 2026-09-20 on Claude Code 2.1.263:** `--help` now prints and documents `case.yaml` and `--no-publish`; a one-case run on Haiku still returns the same early-access refusal. Still unavailable. Its default publishes the report to claude.ai, so `--no-publish` on the day it opens — the cases quote the client repo's documents |

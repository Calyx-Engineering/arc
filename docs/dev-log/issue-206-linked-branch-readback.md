...
| **The keyword test is shell, not jq** | Pass 4 found it recognising `Closes #NN` and none of the three other forms GitHub parses — `GH-NN`, `owner/repo#NN`, the issue URL — each of which would have been read as *no keyword* and printed the one verdict this tool calls decisive. Moving detection out of the GraphQL filter and into `body_has_keyword` is what makes it selftestable; nine of the cases are its |
...
| **Wired into `verify-all.sh`** | Not optional. That script fails on any `tests/verify-*.sh` it does not know, which is what forced the wiring — and is the check working |

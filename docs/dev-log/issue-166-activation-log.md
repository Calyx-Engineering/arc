...
The five registered hooks were the producers that did not exist.

## Decisions & trade-offs

| | |
|---|---|
| **One sourced library, not a line in each hook** | `hooks/lib/activation-log`. Five hooks, thirty-odd exit paths, one entry format — the alternative is five copies that drift, and the format authority is already a file elsewhere |
...
| | |
|---|---|
| `bash tools/verify-all.sh` | **28 gates, all clean**, exit 0 — which runs `verify-hook.sh` against all five hooks, unchanged from before this work |
| `bash tools/verify-activation-log.sh` | **358 passed, 0 failed** — every case in all five case directories, plus the library checks |
...
| Cost per firing | +24ms `mode-guard`, +18ms `tracker-verify`, +24ms `camp-branch-check`, each against the same hook with the `arc_log_*` calls stubbed out, 20 runs each. ~+66ms per Bash tool call, since three of the five are on that matcher |
...
exists"*. Three of the five hooks are on the Bash matcher, so a session appends three
entries per Bash call — `mode-guard` before it, `tracker-verify` and `camp-branch-check` after, most recording that nothing ran. Spawned as

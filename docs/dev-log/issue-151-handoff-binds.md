...
| **Out of scope** | **A live re-run of the 8 openings** — not possible here, because scoring a changed format against a closed corpus of finished transcripts cannot be done: the sessions recorded there read the old format and cannot read another. **It is deferred to no filed issue.** [#181](https://github.com/Calyx-Engineering/arc/issues/181) would supply the capability and the baseline routes it there, but none of #181's boxes re-scores a handoff opening — checked at review pass 3 and recorded in the issue's *Spawned* table instead. **Any format change beyond the two columns and one section named below** — the issue's own constraint, because every previous attempt restyled the document and none produced a score |

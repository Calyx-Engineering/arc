...
| **A gate for the count, never for the content** | `tests/verify-close-sequence.sh` checks only that the Closing table's row count matches every count claim in every live artifact. It cannot see whether a step's text is right — which is the whole reason step 5 exists |
...
carrier for which now exists in `skills/issue-write`. `tests/verify-close-sequence.sh` is the

...
**Issue:** [#239](https://github.com/Calyx-Engineering/arc/issues/239)  ·  **PR:** not yet opened
...
| **Three silences, each deliberate** | No live log, a non-arc branch, and a `Previous:` that names no file. The first is now the normal state of a fresh clone ([#273](https://github.com/Calyx-Engineering/arc/issues/273)), the second is every checkout that is not mid-arc, and failing on either would make the gate noise |

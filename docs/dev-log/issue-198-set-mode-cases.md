...
| **A `tests/verify-set-mode.sh` wrapper** | `verify-all.sh` checks its invocation table against `tests/verify-*.sh` and fails on a verifier it does not know. A gate invoked as `python tools/x.py` sits outside that check entirely. Same shape as the other python-backed gates |

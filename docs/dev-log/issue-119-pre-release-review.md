...
| | |
|---|---|
| **The blocking one is the whole justification for the issue** | 18 template links resolve to nothing where the template lands — 11 of them in the operating agreement, the document m43 requires a user to read and approve. Nothing had ever resolved a template link from its destination, because the templates read correctly where they sit |
| **And it does not reproduce here, which is why it survived** | `.claude/arc/camp/`'s copies use `../../../docs/` and resolve; the templates they came from use `../../`. **Someone fixed the copies by hand and left the template wrong.** Every reader since has seen a working document. A finding visible in the repo would have been caught long ago — this one was only reachable by resolving from a destination that does not exist here |

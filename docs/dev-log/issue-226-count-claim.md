...
| **`closing` ties as a whole word** | Pass 1 found `enclosing` and `disclosing` tying a line. Anchored so that the character before and after `closing` is not a letter |
| **The fixture directory is LF-pinned and excluded from the live scan** | `.gitattributes` pins `tests/close-sequence-cases/**` to LF like the other fixture trees, and the live gate skips it the way it skips the two log trees — its four drifted cases would otherwise fail the real run |

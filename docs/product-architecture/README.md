...
|---|---|---|---|---|---|
| **Workspace guard** | One edit, commit, or PR | Whether *this* action is landing correctly | On every tool call | Edit on the wrong branch · a PR that never links · a commit that drops files | Hooks — mechanical, no judgment |
...
Twenty-six mechanisms across the six pieces.
...
| | **AUTHORING** | | | | |
| 11 | `issue-writing` | ⚙️ | **Issues someone can act on.** *Issue and PR body practice, and the link mechanics that fail silently* | — | ⚪ |
...
| 18 | `engineering-report` | ⚙️ | **Reports that get read.** *Layering, length budgets, and confidence marking* | — | ⚪ |
| 38 | `chat-response` | ⚙️ | **Answers, not essays.** *Length, structure, and when to decide rather than ask* | — | 🔵 |
| | **CAMPAIGN** | | | | |
...
| 22 | Configuration management | 🔥 | **What is in this revision, exactly.** *Software is solved by branches and releases; hardware component and BOM state is not* | — | ⚪ |
...
| 17 | K1 upkeep | ⚙️ | **The always-read files stay current.** *The arc-log status table as work lands, a dev-log per issue filled at decision points* | — | ⚪ |

...
#
#   bash hooks/hooks-off.sh <hook|all> [minutes]   mute; minutes defaults to 30, caps at 480
#   bash hooks/hooks-off.sh status                 what is muted here, and when each lapses
#   bash hooks/hooks-off.sh clear [<hook|all>]     end it now; with no argument, all of it
#   bash hooks/hooks-off.sh selftest               the gate
#
...
# lapses. One instruction works in PowerShell, cmd and bash — `bash hooks/hooks-off.sh` — so
...
# This file lives BESIDE the hooks, not in `tools/`, and that is the whole of its
# reachability. Arc installs as a plugin, and an installed plugin ships `hooks/` — a consumer
# repository has no `tools/` directory at all, so a kill switch documented as
# `bash tools/hooks-off.sh …` would be the 2026-09-07 failure relocated rather than removed:
# a line typed under pressure that silently does nothing. `tools/hooks-off.sh` is a delegator
# kept for the one repository that has it, this one.
...
if ! . "$HERE/lib/hooks-off" 2>/dev/null; then
  echo "hooks-off: cannot read $HERE/lib/hooks-off — is this a complete checkout?" >&2
  exit 2
fi
...
known_hooks() {
  ...
  for f in "$HOOKS_DIR"/*; do
    [ -f "$f" ] || continue
    case "${f##*/}" in TEMPLATE|hooks.json|*.md|*.json|*.sh) continue ;; esac
    printf '%s\n' "${f##*/}"
  done
}
...
selftest() {
  ...
  # Clearing one hook while `all` is live leaves it muted. Saying "it is on again" there is
  # false in the direction that gets a guard trusted while it is off.
  ( cd "$T" && bash "$HERE/hooks-off.sh" all 30 >/dev/null 2>&1 )
  ( cd "$T" && bash "$HERE/hooks-off.sh" branch-guard 30 >/dev/null 2>&1 )
  out="$( cd "$T" && bash "$HERE/hooks-off.sh" clear branch-guard 2>&1 )"
  if printf '%s' "$out" | grep -q 'NOT on yet'; then
    ok "clearing one hook under a live \`all\` says so rather than claiming it is on"
  else
    bad "clearing one hook under a live \`all\` says so rather than claiming it is on" "$out"
  fi
  ( cd "$T" && bash "$HERE/hooks-off.sh" clear >/dev/null 2>&1 )

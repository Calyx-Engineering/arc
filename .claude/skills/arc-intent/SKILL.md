...
camp-reports: [intent-checked, drift-escalated]
checks: [arc-log-read, intent-located, classification, decision-honoured]
skips:
  - intent-located (the current arc has no arc-log)
---
...
individual step appears reasonable. Nobody notices, because nobody is holding the
destination — and by the time the divergence is visible it has been paid for.

| | |
|---|---|
| **Is** | Holding the destination, and evaluating proposed work against it |
| **Is not** | Reading history to locate past errors — that is the retrospective |
...
that has gone too deep. This catches work going somewhere the arc did not agree to go.
...
If the current arc has no `arc-log`, **say so and do not classify.** An answer with no stated
intent behind it is the improvisation this obligation exists to prevent — record
`intent-located` as skipped and let the work proceed.
...
of the intent rather than a restatement of it; a two-level ladder classifies all of it as
drift and is then ignored.
...
**Camp cannot revise an arc's intent; only the user can.** That constraint is what makes this
a delegate rather than an observer — the flag cannot be argued away by the thing that proposed
the work, and the destination cannot be quietly moved to match where the work went.

An arc whose intent has genuinely changed gets its `arc-log` amended by the user, in a diff.
...
| **An issue closes · a PR opens** | [`issue-write`](../issue-write/SKILL.md) | Is what merged the work the arc asked for |
...
There, direction **tunes the nudge rather than gating it**:
...
> **A fired precondition always produces a nudge.** Direction sets its strength, never whether
> it speaks.
...
Every classification is reported, **including *agreed***.
...
**An obligation that only speaks on escalate is indistinguishable from one that never ran.**
State what was checked, not only what was found —
[`camp-reports.md`](../../docs/product-architecture/camp-reports.md).

At `quiet`, only an *escalate* surfaces. It still reaches the event log at every level.

---

## Worked case

Five issues about how issues and chat responses are written — [#31](https://github.com/Calyx-Engineering/arc/issues/31) · [#32](https://github.com/Calyx-Engineering/arc/issues/32) · [#33](https://github.com/Calyx-Engineering/arc/issues/33) · [#34](https://github.com/Calyx-Engineering/arc/issues/34) · [#35](https://github.com/Calyx-Engineering/arc/issues/35) — were filed during an arc
scoped to Camp.

| | |
|---|---|
| **Classification** | *Escalate.* Tracker mechanics, and the arc's stated intent is Camp |
| **User decision** | Proceed — the work is real and the friction was live |
| **Outcome** | Filed, scheduled as wave 5, and not raised again |
...
The three call sites limit how much this matters — `decompose`, `issue-write` and
`relief-valve` fire it from a mechanical moment rather than from self-awareness. **It does not
remove it.** The independent form is an observer outside the conversation, deferred in
[m43 §4](../../docs/product-architecture/mechanisms/m43-camp-assistant.md) for the same reason
the relief valve's is: continuous conversation reading is the one cost Arc cannot currently
carry.
